...
GENESIS_FILE="$HOME"/.practice/config/genesis.json
if [ -f "$GENESIS_FILE" ]; then
  echo "$GENESIS_FILE exists..."
else
  echo "$GENESIS_FILE does not exist. Generating..."

  practiced init --chain-id "$CHAIN_ID" "$MONIKER"
  practiced add-ica-config
  ...
  sed -i 's/keyring-backend = "os"/keyring-backend = "test"/' "$HOME"/.practice/config/client.toml
fi

APP_TOML_CONFIG="$HOME"/.practice/config/app.toml
APP_TOML_CONFIG_NEW="$HOME"/.practice/config/app_new.toml
CONFIG_TOML_CONFIG="$HOME"/.practice/config/config.toml
...
if ! practiced keys show validator $KEYRING; then
  (echo "$PASSWORD"; echo "$PASSWORD") | practiced keys add validator $KEYRING

  # hardcode the validator account for this instance
  echo "$PASSWORD" | practiced add-genesis-account validator "1000000000$STAKE,1000000000$FEE" $KEYRING

  # (optionally) add a few more genesis accounts
  for addr in "$@"; do
    echo $addr
    practiced add-genesis-account "$addr" "1000000000$STAKE,1000000000$FEE"
  done

  # submit a genesis validator tx
  ## Workraround for https://github.com/cosmos/cosmos-sdk/issues/8251
  (echo "$PASSWORD"; echo "$PASSWORD"; echo "$PASSWORD") | practiced gentx validator "250000000$STAKE" --chain-id="$CHAIN_ID" --amount="250000000$STAKE" $KEYRING
  ## should be:
  # (echo "$PASSWORD"; echo "$PASSWORD"; echo "$PASSWORD") | practiced gentx validator "250000000$STAKE" --chain-id="$CHAIN_ID"
