...
depending on the practice codebase. It initializes the chains for testing
via Docker files. As a result, the test suite may provide the desired
practice version to Docker containers during the initialization. This
design allows for the opportunity of testing chain upgrades in the
future by providing an older practice version to the container,
...
1. Chain Initialization
    ...
    of the previous practice version, inside `configurer/upgrade.go`. This is
    ...
    current branch version of practice codebase
    ...
    with IBC initialized at the previous practice version
...
2. Setting up e2e components
    ...
    - IBC testing
        ...
        - Upgrade Testing
        - 2 chains of the older practice version are created, and
    ...
    - Upgrade testing
        ...
        - Current branch practice version is spun up to continue with testing
...
such as the `app.toml`. This package directly depends on the practice
codebase.
...
production practice container to start the `practiced` service.
...
### To build the debug practice image
...
- `practice_E2E_SKIP_UPGRADE` - when true, skips the upgrade tests.
If practice_E2E_SKIP_IBC is true, this must also be set to true because upgrade
tests require IBC logic.

- `practice_E2E_SKIP_IBC` - when true, skips the IBC tests tests.

- `practice_E2E_SKIP_STATE_SYNC` - when true, skips the state sync tests.

- `practice_E2E_SKIP_CLEANUP` - when true, avoids cleaning up the e2e Docker
containers.

- `practice_E2E_FORK_HEIGHT` - when the above "IS_FORK" env variable is set to true, this is the string
of the height in which the network should fork. This should match the ForkHeight set in constants.go

- `practice_E2E_UPGRADE_VERSION` - string of what version will be upgraded to (for example, "v10")

- `practice_E2E_DEBUG_LOG` - when true, prints debug logs from executing CLI commands
...
```json
{
    ...
    "buildFlags": "-tags e2e",
    "env": {
        "practice_E2E_SKIP_IBC": "true",
        "practice_E2E_SKIP_UPGRADE": "true",
        "practice_E2E_SKIP_CLEANUP": "true",
        "practice_E2E_SKIP_STATE_SYNC": "true",
        "practice_E2E_UPGRADE_VERSION": "v11",
        "practice_E2E_DEBUG_LOG": "true",
        "practice_E2E_FORK_HEIGHT": "42345124" 
    }
}
```
