...
import (
	...
	"github.com/BOSEES/practice/v12/tests/e2e/initialization"
...
func (s *IntegrationTestSuite) TestIBCTokenTransfer() {
	...
	chainA.SendIBC(chainB, chainB.NodeConfigs[0].PublicAddress, initialization.practiceToken)
	chainB.SendIBC(chainA, chainA.NodeConfigs[0].PublicAddress, initialization.practiceToken)
...
func (s *IntegrationTestSuite) TestTokenFactoryBindings() {
	...
	// Store Contract
	node.StoreWasmCode("/practice/tokenfactory.wasm", wallet)
	...
	ourDenom := fmt.Sprintf("factory/%s/test", contractAddr) // factory/practice14hj2tavq8fpesdwxxcu44rty3hh90vhujrvcmstl4zr3txmfvw9skjuwg8/test
	...
	msg = fmt.Sprintf(`{"change_admin":{"denom":"%s","new_admin_address":"%s"}}`, ourDenom, "practice1aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaavju90c")
	node.WasmExecute(contractAddr, msg, wallet, "", tfSuccessCode)
}
