...
func NewManager(isUpgrade bool, isFork bool, isDebugLogEnabled bool) (docker *Manager, err error) {
	...
	docker.network, err = docker.pool.CreateNetwork("practice-testnet")
	if err != nil {
		return nil, err
	}
	return docker, nil
...
func (m *Manager) RunHermesResource(chainAID, practiceARelayerNodeName, practiceAValMnemonic, chainBID, practiceBRelayerNodeName, practiceBValMnemonic string, hermesCfgPath string) (*dockertest.Resource, error) {
	hermesResource, err := m.pool.RunWithOptions(
		&dockertest.RunOptions{
			...
			Env: []string{
				fmt.Sprintf("practice_A_E2E_CHAIN_ID=%s", chainAID),
				fmt.Sprintf("practice_B_E2E_CHAIN_ID=%s", chainBID),
				fmt.Sprintf("practice_A_E2E_VAL_MNEMONIC=%s", practiceAValMnemonic),
				fmt.Sprintf("practice_B_E2E_VAL_MNEMONIC=%s", practiceBValMnemonic),
				fmt.Sprintf("practice_A_E2E_VAL_HOST=%s", practiceARelayerNodeName),
				fmt.Sprintf("practice_B_E2E_VAL_HOST=%s", practiceBRelayerNodeName),
			},
			Entrypoint: []string{
				"sh",
				"-c",
...
func (m *Manager) RunNodeResource(chainID string, containerName, valCondifDir string) (*dockertest.Resource, error) {
	...
	runOpts := &dockertest.RunOptions{
		Name:       containerName,
		Repository: m.practiceRepository,
		Tag:        m.practiceTag,
		NetworkID:  m.network.Network.ID,
		User:       "root:root",
		Cmd:        []string{"start", "--trace"},
		Mounts: []string{
			fmt.Sprintf("%s/:/practice/.practice", valCondifDir),
			fmt.Sprintf("%s/scripts:/practice", pwd),
		},
	}
