...
import (
	...
	"github.com/BOSEES/practice/v12/tests/e2e/util"
)
...
const (
	...
	// chainA
	ChainAID         = "practice-test-a"
	practiceBalanceA = 200000000000
	StakeBalanceA    = 110000000000
	StakeAmountA     = 100000000000
	// chainB
	ChainBID         = "practice-test-b"
	practiceBalanceB = 500000000000
...
var (
	...
	InitBalanceStrA = fmt.Sprintf("%d%s,%d%s", practiceBalanceA, BaseDenom, StakeBalanceA, StakeDenom)
	InitBalanceStrB = fmt.Sprintf("%d%s,%d%s", practiceBalanceB, BaseDenom, StakeBalanceB, StakeDenom)
	practiceToken   = sdk.NewInt64Coin(BaseDenom, IbcSendAmount)  // 3,300upractice
...
func initGenesis(chain *internalChain, votingPeriod time.Duration, forkHeight int) error {
	...
	cost, _ := sdk.ParseCoinsNormalized("1000000upractice,10000000stake")
