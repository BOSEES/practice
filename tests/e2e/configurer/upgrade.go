...
import (
	...
	"github.com/BOSEES/practice/v12/tests/e2e/configurer/chain"
	"github.com/BOSEES/practice/v12/tests/e2e/configurer/config"
	"github.com/BOSEES/practice/v12/tests/e2e/containers"
...
func (uc *UpgradeConfigurer) ConfigureChain(chainConfig *chain.Config) error {
	...
	tmpDir, err := os.MkdirTemp("", "practice-e2e-testnet-")
	if err != nil {
		return err
	}
...
func (uc *UpgradeConfigurer) upgradeContainers(chainConfig *chain.Config, propHeight int64) error {
	...
	uc.containerManager.practiceRepository = containers.CurrentBranchRepository
	uc.containerManager.practiceTag = containers.CurrentBranchTag
