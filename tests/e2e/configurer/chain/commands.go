...
import (
	...
	appparams "github.com/BOSEES/practice/v12/app/params"
	"github.com/BOSEES/practice/v12/tests/e2e/configurer/config"
...
func (n *NodeConfig) StoreWasmCode(wasmFile, from string) {
	...
	cmd := []string{"practiced", "tx", "wasm", "store", wasmFile, fmt.Sprintf("--from=%s", from), "--gas=5000000", "--gas-prices=0.1upractice", "--gas-adjustment=1.5", "--output=json"}
...
func (n *NodeConfig) InstantiateWasmContract(codeID, initMsg, label, from, admin string) {
	...
	cmd := []string{"practiced", "tx", "wasm", "instantiate", codeID, initMsg, fmt.Sprintf("--from=%s", from), "--gas=5000000", fmt.Sprintf("--label=%s", label)}
...
func (n *NodeConfig) WasmExecute(contract, execMsg, from, amounts string, successString string) {
	...
	cmd := []string{"practiced", "tx", "wasm", "execute", contract, execMsg, fmt.Sprintf("--from=%s", from)}
	if len(amounts) > 0 {
...
func (n *NodeConfig) QueryParams(subspace, key string, result any) {
	cmd := []string{"practiced", "query", "params", "subspace", subspace, key, "--output=json"}
...
func (n *NodeConfig) SubmitParamChangeProposal(proposalJSON, from string) {
	...
	cmd := []string{"practiced", "tx", "gov", "submit-proposal", "param-change", "/practice/param_change_proposal.json", fmt.Sprintf("--from=%s", from)}
...
func (n *NodeConfig) FailIBCTransfer(from, recipient, amount string) {
	...
	cmd := []string{"practiced", "tx", "ibc-transfer", "transfer", "transfer", "channel-0", recipient, amount, fmt.Sprintf("--from=%s", from)}
...
func (n *NodeConfig) SubmitTextProposal(text string, initialDeposit sdk.Coin) {
	...
	cmd := []string{"practiced", "tx", "gov", "submit-proposal", "--type=text", fmt.Sprintf("--title=\"%s\"", text), "--description=\"test text proposal\"", "--from=val", fmt.Sprintf("--deposit=%s", initialDeposit)}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainID, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) DepositProposal(proposalNumber int, isExpedited bool) {
	...
	cmd := []string{"practiced", "tx", "gov", "deposit", fmt.Sprintf("%d", proposalNumber), deposit, "--from=val"}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainID, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) VoteYesProposal(from string, proposalNumber int) {
	...
	cmd := []string{"practiced", "tx", "gov", "vote", fmt.Sprintf("%d", proposalNumber), "yes", fmt.Sprintf("--from=%s", from)}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainID, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) VoteNoProposal(from string, proposalNumber int) {
	...
	cmd := []string{"practiced", "tx", "gov", "vote", fmt.Sprintf("%d", proposalNumber), "no", fmt.Sprintf("--from=%s", from)}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainID, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) BankSend(amount string, sendAddress string, receiveAddress string) {
	...
	cmd := []string{"practiced", "tx", "bank", "send", sendAddress, receiveAddress, amount, "--from=val"}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainID, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) CreateWallet(walletName string) string {
	...
	cmd := []string{"practiced", "keys", "add", walletName, "--keyring-backend=test"}
	outBuf, _, err := n.containerManager.ExecCmd(n.t, n.Name, cmd, "")
	require.NoError(n.t, err)
	re := regexp.MustCompile("practice1(.{38})")
...
func (n *NodeConfig) GetWallet(walletName string) string {
	...
	cmd := []string{"practiced", "keys", "show", walletName, "--keyring-backend=test"}
	outBuf, _, err := n.containerManager.ExecCmd(n.t, n.Name, cmd, "")
	require.NoError(n.t, err)
	re := regexp.MustCompile("practice1(.{38})")
	walletAddr := fmt.Sprintf("%s\n", re.FindString(outBuf.String()))
...
func (n *NodeConfig) QueryPropStatusTimed(proposalNumber int, desiredStatus string, totalTime chan time.Duration) {
	...
	require.Eventually(
		...
		1*time.Minute,
		10*time.Millisecond,
		"practice node failed to retrieve prop tally",
	)
...
func (n *NodeConfig) SubmitUpgradeProposal(upgradeVersion string, upgradeHeight int64, initialDeposit sdk.Coin) {
	...
	cmd := []string{"practiced", "tx", "gov", "submit-proposal", "software-upgrade", upgradeVersion, fmt.Sprintf("--title=\"%s upgrade\"", upgradeVersion), "--description=\"upgrade proposal submission\"", fmt.Sprintf("--upgrade-height=%d", upgradeHeight), "--upgrade-info=\"\"", "--from=val", fmt.Sprintf("--deposit=%s", initialDeposit)}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainID, n.Name, cmd)
	require.NoError(n.t, err)
